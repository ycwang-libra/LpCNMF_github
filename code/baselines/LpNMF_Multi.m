function [U_final, Z_final,F_final, nIter_final, obj_all] = LpNMF_Multi(X, A, r, k, S, W, options, U, Z, F)
    % Label Propagation NMF(LpNMF)
    %
    % where
    %   X
    % Notation:
    ...
    function [U,Z,F] = NormalizeUV(U,Z,F,NormF,Norm)
        if Norm == 2   %2范数            
            if NormF %对F归一化
                ...
                Z = spdiags(sqrt(norms),0,size(F,2),size(F,2)) * Z * spdiags(sqrt(normsu),0,size(Z,2),size(Z,2));  % k r 
            end
        else
            if NormF
