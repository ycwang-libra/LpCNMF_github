% demo regularization with all datasets for acc, NMI, purity, F-score
clc;
clear;
close all;
...
root_path = 'code_and_datasets/Saved_Results/regularization_results/';
...
Range_r=[0.001,0.01,0.1,1,10,100,1000];
percent = 0.3;
alpha = 1;
nCase  = length(nCluster);  % 9
num_regularization = length(Range_r);
nRun = 10; 
maxIter = 100;

ACC=zeros(nCase,num_regularization,nRun);
NMI=zeros(nCase,num_regularization,nRun);
PUR=zeros(nCase,num_regularization,nRun);
FSC=zeros(nCase,num_regularization,nRun);

tic;
fprintf('Regul  0.001   0.01     0.1       1     10      100    1000 \n');
...
for k = nCluster
    ...
    for r = Range_r
        ...
        for runIter = 1:nRun 
            ...
            nSmpLabeled = sum(semiSplit); % 500k * percent(0.3) = 300
...
Ave_ac = mean(ACC_mean,1); % caseIter mean
fprintf('AVE  ');
fprintf(' %3.3f  ',Ave_ac);
fprintf('\n');
fprintf('best_r  ');
fprintf('%3.3f ', Range_r(Ave_ac==max(Ave_ac)));
...
xlabel('Regulation Factor r');
ylabel('ACC');
title([dataset,'-Regularization']);
